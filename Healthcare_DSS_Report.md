...
کتاب «تحلیل‌ها، علم داده و هوش مصنوعی: سیستم‌های پشتیبانی تصمیم‌گیری»[^11] نوشته توربان، شاردا و دلن چارچوب جامعی برای طراحی و پیاده‌سازی سیستم‌های پشتیبانی تصمیم‌گیری ارائه می‌دهد. این چارچوب بر اساس مدل چهار مرحله‌ای هربرت سایمون[^15] شامل مراحل هوشمندی[^16]، طراحی[^17]، انتخاب[^18] و پیاده‌سازی[^19] است.

در این پروژه، یک سیستم پشتیبانی تصمیم‌گیری جامع برای مراقبت‌های بهداشتی[^2] پیاده‌سازی شده است که از چهار زیرسیستم اصلی تشکیل شده: مدیریت داده‌ها[^20]، مدیریت مدل‌ها[^21]، مدیریت دانش[^22] و رابط کاربری[^6]. سیستم با استفاده از الگوریتم‌های یادگیری ماشین[^7] مختلف شامل جنگل تصادفی[^23]، تقویت گرادیان شدید[^24]، تقویت گرادیان سبک[^25] و ماشین بردار پشتیبان[^26]، قابلیت تحلیل و پیش‌بینی در حوزه‌های مختلف بهداشتی را فراهم می‌آورد.

هدف اصلی این پروژه، حل مسائل عملی در سیستم‌های بهداشتی از طریق ایجاد پلتفرمی یکپارچه[^27] است که بتواند داده‌های مختلف بهداشتی را پردازش کرده و بینش‌های عملی[^28] برای تصمیم‌گیری‌های بالینی[^29] و مدیریتی[^30] ارائه دهد.

### مسائل حل شده توسط سیستم

سیستم طراحی شده برای حل چهار دسته مسئله اصلی در مراقبت‌های بهداشتی طراحی شده است:

**۱. مسائل تشخیص و پیش‌بینی بالینی**: شامل تشخیص زودهنگام[^31] سرطان پستان[^32], پیش‌بینی پیشرفت دیابت[^33]، و ارزیابی ریسک[^34] بستری مجدد بیماران که از طریق داشبورد کادر بالینی[^35] قابل دسترسی است.

**۲. مسائل مدیریت منابع و ظرفیت**: شامل بهینه‌سازی ظرفیت[^36] بیمارستان، تخصیص بهینه[^37] تخت‌ها، و مدیریت جریان بیماران[^38] که از طریق داشبورد مدیریت اجرایی[^39] پشتیبانی می‌شود.

**۳. مسائل ارزیابی عملکرد**: شامل نظارت بر عملکرد کادر[^40]، ارزیابی کیفیت بخش‌ها[^41]، و تحلیل رضایت بیماران[^42] که از طریق داشبورد مدیر بخش[^43] قابل پیگیری است.

**۴. مسائل مالی و استراتژیک**: شامل تحلیل هزینه‌های بهداشتی[^44]، پیش‌بینی درآمد[^45]، و بهینه‌سازی بودجه[^46] که از طریق داشبورد مدیر مالی[^47] و رهبری بحرانی[^48] پشتیبانی می‌شود.

**ویژگی‌های کلیدی سیستم**:
- پشتیبانی از ۱۲ مجموعه‌داده[^49] مختلف بهداشتی (۶ واقعی[^50]، ۶ مصنوعی[^51])
- پیاده‌سازی ۸ الگوریتم یادگیری ماشین[^7] شامل جنگل تصادفی[^23]، تقویت گرادیان شدید[^24]، تقویت گرادیان سبک[^25]، ماشین بردار پشتیبان[^26]، رگرسیون لجستیک[^52]، رگرسیون خطی[^53]، درخت تصمیم[^54] و بیز ساده[^55]
- ۵ داشبورد تخصصی[^56] برای نقش‌های مختلف شامل کادر بالینی[^35]، مدیریت اجرایی[^39]، رهبری بحرانی[^48]، مدیر بخش[^43] و مدیر مالی[^47]
- سیستم ارزیابی جامع[^57] با ۱۵۹ تست (نرخ موفقیت ۸۹.۹۴٪)
- معماری مدولار[^58] شامل ۴ زیرسیستم اصلی
- پشتیبانی از ۲۳ کتابخانه تخصصی[^59] پایتون[^60]
...
[^15]: Herbert Simon's Four-Phase Decision Model
[^16]: Intelligence Phase
[^17]: Design Phase
[^18]: Choice Phase
[^19]: Implementation Phase
[^20]: Data Management Subsystem
[^21]: Model Management Subsystem
[^22]: Knowledge Management Subsystem
[^23]: Random Forest
[^24]: XGBoost (Extreme Gradient Boosting)
[^25]: LightGBM (Light Gradient Boosting Machine)
[^26]: Support Vector Machine (SVM)
[^27]: Integrated Platform
[^28]: Practical Insights
[^29]: Clinical Decision Making
[^30]: Management Decision Making
[^31]: Early Detection
[^32]: Breast Cancer
[^33]: Diabetes Progression Prediction
[^34]: Risk Assessment
[^35]: Clinical Staff Dashboard
[^36]: Capacity Optimization
[^37]: Optimal Allocation
[^38]: Patient Flow Management
[^39]: Administrative Executive Dashboard
[^40]: Staff Performance Monitoring
[^41]: Department Quality Assessment
[^42]: Patient Satisfaction Analysis
[^43]: Department Manager Dashboard
[^44]: Healthcare Cost Analysis
[^45]: Revenue Prediction
[^46]: Budget Optimization
[^47]: Financial Manager Dashboard
[^48]: Critical Leadership Dashboard
[^49]: Datasets
[^50]: Real-world Data
[^51]: Synthetic Data
...
[^54]: Decision Tree
[^55]: Naive Bayes
[^56]: Specialized Dashboards
[^57]: Comprehensive Evaluation System
[^58]: Modular Architecture
[^59]: Specialized Libraries
...
**رویکرد اول - چارچوب نظری**: پیاده‌سازی بر اساس چارچوب نظری کتاب توربان[^3]، شاردا و دلن و مدل چهار مرحله‌ای هربرت سایمون[^5] برای تصمیم‌گیری انجام شده است.

**رویکرد دوم - روش‌شناسی کریسپ-دی‌ام**: برای توسعه مدل‌های یادگیری ماشین[^10] از روش‌شناسی شش مرحله‌ای کریسپ-دی‌ام[^27] استفاده شده که شامل درک کسب‌وکار، درک داده‌ها، آماده‌سازی داده‌ها، مدل‌سازی، ارزیابی و استقرار است.

**رویکرد سوم - آزمایش تجربی**: برای اعتبارسنجی فرضیه‌های تحقیق، طرح آزمایشی جامع طراحی شده که شامل:
- آزمایش عملکرد ۸ الگوریتم یادگیری ماشین[^10] بر روی ۱۲ مجموعه‌داده مختلف
...
- **دیابت**[^29] (۴۴۲ نمونه، ۱۰ ویژگی): پیش‌بینی پیشرفت بیماری دیابت[^29] با متغیر هدف (نوع رگرسیون[^31])
  - ویژگی‌های کلیدی: سن[^32]، جنس[^33]، شاخص توده بدنی[^34]، فشار خون[^35]، شش متغیر سرمی[^36] نرمال‌سازی شده
  - متغیر هدف اصلی: مقدار پیوسته ۲۱۴ مقدار منحصربه‌فرد، دامنه ۲۵-۳۴۶
  - عملکرد بهترین مدل: رگرسیون خطی با ضریب تعیین ۰.۴۵۳
  
- **سرطان پستان**[^37] (۵۶۹ نمونه، ۳۰ ویژگی): تشخیص سرطان پستان[^37] با دقت بالا
...
- **شراب**[^45] (۱۷۸ نمونه، ۱۳ ویژگی): طبقه‌بندی انواع شراب برای تحقیقات پزشکی[^45]
...
- **ظرفیت بیمارستان**[^59] (۲۰,۶۴۰ نمونه، ۸ ویژگی): داده‌های ظرفیت بیمارستان[^59]
...
- **هزینه‌های بهداشتی جهانی**[^65] (۱۱۷ نمونه، ۱۴ ویژگی): هزینه‌های بهداشتی[^65] ۱۶ کشور از ۲۰۱۵ تا ۲۰۲۴
...
- **جمعیت‌شناسی بیماران**[^68] (۱,۰۰۰ نمونه، ۲۰ ویژگی): ویژگی‌های جمعیت‌شناختی بیماران[^68]
...
- **نتایج بالینی**[^80] (۱,۰۰۰ نمونه، ۷ ویژگی): نتایج درمان و رضایت بیماران[^80]
...
- **عملکرد کادر**[^82] (۲۰۰ نمونه، ۹ ویژگی): ارزیابی عملکرد کارکنان[^82]
...
- **عملکرد بخش‌ها**[^87] (۷ نمونه، ۸ ویژگی): کارایی ۷ بخش مختلف بیمارستان[^87]
...
- **اثربخشی دارو**[^102] (۱۷۸ نمونه، ۱۴ ویژگی): تحلیل اثربخشی داروها[^102]
...
- `bmi`: شاخص توده بدنی[^34] (نرمال‌سازی شده)
...
[^132]: Standard Scaler
[^133]: Synthetic Minority Oversampling Technique (SMOTE)
...
- **روش حل**: استفاده از مدل‌های طبقه‌بندی (رگرسیون لجستیک[^16]، جنگل تصادفی[^11]) برای تشخیص سرطان پستان[^37] و پیش‌بینی پیشرفت دیابت[^29]
- **معیارهای ارزیابی**: دقت تشخیص ۹۸.۲٪ برای سرطان پستان[^37]
...
- **مسئله**: بهینه‌سازی استفاده از منابع و نظارت بر عملکرد کادر[^82]
- **روش حل**: استفاده از مدل‌های رگرسیون[^31] برای پیش‌بینی عملکرد کادر[^82] و تحلیل رضایت بیماران
...
- **روش حل**: تحلیل سری زمانی هزینه‌های بهداشتی جهانی[^65] ۱۶ کشور طی ۱۰ سال
...
- **روش حل**: تحلیل عملکرد بخش‌ها[^87] ۷ بخش تخصصی (اورژانس[^89]، قلب[^90]، انکولوژی[^91]، اطفال[^92]، جراحی[^93]، مراقبت‌های ویژه[^94]، رادیولوژی[^95])
...
[^172]: Coefficient of Determination ($R^2$)
...
### ارزیابی فنی مدل‌ها
- **معیارهای طبقه‌بندی**: دقت[^165]، دقت مثبت[^166]، حساسیت[^167]، امتیاز اف‌یک[^168]، سطح زیر منحنی راک[^169]
- **معیارهای رگرسیون**: ریشه میانگین مربعات خطا[^170]، میانگین قدر مطلق خطا[^171]، ضریب تعیین[^172]، میانگین درصد خطای مطلق[^173]
...
**مجموعه‌داده دیابت[^29] (۴۴۲ نمونه، ۱۰ ویژگی، رگرسیون[^31])**:
- **بهترین مدل**: رگرسیون خطی[^17] با $R^2 = 0.453$
- **جنگل تصادفی[^11]**: $R^2 = 0.443$، $RMSE = 54.332$، $CV_{mean} = 0.429 \pm 0.084$
- **رگرسیون خطی[^17]**: $R^2 = 0.453$، $RMSE = 53.853$، $CV_{mean} = 0.478 \pm 0.085$
- **درخت تصمیم[^18]**: $R^2 = 0.061$، $RMSE = 70.546$، $CV_{mean} = -0.133 \pm 0.133$

**مجموعه‌داده سرطان پستان[^37] (۵۶۹ نمونه، ۳۰ ویژگی، طبقه‌بندی)**:
- **بهترین مدل**: رگرسیون لجستیک[^16] با دقت $98.2\%$
- **جنگل تصادفی[^11]**: دقت = $95.6\%$، $F_1 = 0.956$، $CV_{mean} = 0.956 \pm 0.012$
- **رگرسیون لجستیک**: دقت = $98.2\%$، $F_1 = 0.982$، $CV_{mean} = 0.940 \pm 0.017$
- **درخت تصمیم[^18]**: دقت = $91.2\%$، $F_1 = 0.913$، $CV_{mean} = 0.910 \pm 0.028$

**مجموعه‌داده شراب[^45] (۱۷۸ نمونه، ۱۳ ویژگی، طبقه‌بندی)**:
- **بهترین مدل**: جنگل تصادفی[^11] با دقت $100\%$
- **جنگل تصادفی[^11]**: دقت = $100\%$، $F_1 = 1.000$، $CV_{mean} = 0.977 \pm 0.021$
- **رگرسیون لجستیک[^16]**: دقت = $97.2\%$، $F_1 = 0.972$، $CV_{mean} = 0.955 \pm 0.022$
- **درخت تصمیم[^18]**: دقت = $94.4\%$، $F_1 = 0.945$، $CV_{mean} = 0.893 \pm 0.038$

**مجموعه‌داده ظرفیت بیمارستان[^59] (۲۰,۶۴۰ نمونهیژگی، رگرسیون)**:)**:
- **بهترین مدل**: جنگل تصادفی[^11] با $R^2 = 0.805$
- **جنگل تصادفی[^11]**: $R^2 = 0.805$، $RMSE = 0.506$، $CV_{mean} = 0.810 \pm 0.007$
- **رگرسیون خطی[^17]**: $R^2 = 0.576$، $RMSE = 0.746$، $CV_{mean} = 0.601 \pm 0.017$
- **درخت تصمیم[^18]**: $R^2 = 0.619$، $RMSE = 0.707$، $CV_{mean} = 0.614 \pm 0.006$
...
[^206]: Maximum Accuracy
[^207]: Minimum Accuracy
...
- تجسم‌ها: سنجه[^300]، نمودار خطی[^301]، ماتریس درهم‌ریختگی[^302]
...
- تجسم‌ها: نمودار میله‌ای[^303]، نمودار خطی[^301]، نقشه حرارتی[^304]
...
- تجسم‌ها: کارت‌های شاخص کلیدی عملکرد[^305]، سنجه[^300]، نمودار روند[^306]
...
**معماری سیستم**:
- **زبان برنامه‌نویسی**: پایتون[^25] ۳.۱۳
- **پایگاه داده**: اس‌کیولایت[^119] با پشتیبانی چندنخی[^120]
- **معماری**: مدولار با چهار زیرسیستم اصلی
- **الگوی طراحی**: ام‌وی‌سی[^307] (مدل-نما-کنترلر)
- **روش‌شناسی**: کریسپ-دی‌ام[^27] و مدل چهار مرحله‌ای هربرت سایمون[^3]
...
- **ساختار داده**: نام سری[^308]، کد سری[^309]، نام کشور[^310]، کد کشور[^311] + ۱۰ ستون سالانه
...
پروژه سیستم پشتیبانی تصمیم‌گیری مراقبت‌های بهداشتی با موفقیت قابل‌توجه پیاده‌سازی شده و نتایج ملموسی حاصل کرده است. سیستم با ترکیب چهار زیرسیستم اصلی (مدیریت داده‌ها[^6]، مدیریت مدل‌ها[^7]، مدیریت دانش[^8] و رابط کاربری[^9])، پلتفرمی جامع و کاربردی برای پشتیبانی تصمیمات بهداشتی[^2] ارائه داده است که بر اساس استانداردهای علمی و چارچوب‌های معتبر طراحی شده است.

**دستاوردهای کلیدی**:
- **پیاده‌سازی کامل سیستم**: ۱۲ مجموعه‌داده مختلف بهداشتی با ۲۴,۰۰۰+ رکورد (بزرگترین: ۲۰,۶۴۰ رکورد ظرفیت بیمارستان)
- **عملکرد بالای مدل‌ها**: دقت ۹۸.۲٪ در تشخیص سرطان پستان[^37]، ۱۰۰٪ در طبقه‌بندی شراب[^45]، $R^2 = 0.805$ در پیش‌بینی ظرفیت بیمارستان[^59]
...
- **پیاده‌سازی روش‌شناسی**: کریسپ-دی‌ام[^27] و مدل چهار مرحله‌ای هربرت سایمون با کلاس‌های تخصصی
- **تنوع الگوریتم‌ها**: ۸ الگوریتم یادگیری ماشین[^10] با ۲۴ مدل آموزش‌دی
...
- **بهترین الگوریتم‌ها**: جنگل تصادفی[^11] برای مجموعه‌داده‌های بزرگ، رگرسیون خطی[^17] و لجستیک[^16] برای داده‌های ساده
...
- **چارچوب توربان[^3]**: تحقق چهار زیرسیستم اصلی سیستم پشتیبانی تصمیم‌گیری[^1] مطابق کتاب مرجع[^3]
...
├── utils/         # ابزارها و گردش کار کریسپ-دی‌ام[^27]
...
1. **توربان، ای.، شاردا، آر.، و دلن، د.** (۲۰۲۰). تحلیل‌ها، علم داده و هوش مصنوعی: سیستم‌های پشتیبانی تصمیم‌گیری (ویرایش یازدهم). پیرسون[^3].
...
3. **فرایند استاندارد صنعتی برای داده‌کاوی (کریسپ-دی‌ام)** (۲۰۰۰)[^27].
...
5. **کتابخانه‌های یادگیری ماشین**:
   ...
   - **ایکس‌جی‌بوست**[^12] (>=۱.۷.۰): https://xgboost.readthedocs.io/
   - **لایت‌جی‌بی‌ام**[^13] (>=۴.۰.۰): https://lightgbm.readthedocs.io/
...
[^303]: Bar Chart
[^304]: Heatmap
[^305]: Key Performance Indicator (KPI) Cards
[^306]: Trend Chart
[^307]: Model-View-Controller (MVC)
[^308]: Series Name
[^309]: Series Code
[^310]: Country Name
[^311]: Country Code
